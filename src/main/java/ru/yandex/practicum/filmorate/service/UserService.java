...
public class UserService {
    ...
    public void addFriend(Integer id, Integer friendsId) {
        ...
        if (userStorage.findUserById(friendsId) == null) {
        ...
        User anotherUser = userStorage.findUserById(friendsId);

        if (!user.getFriends().contains(friendsId)) {
            user.getFriends().add(friendsId);
            anotherUser.getFriends().add(id);
    ...
    public List<User> getMutualFriendsSet(Integer id, Integer friendId) {
        Set<Integer> usersFriendsSet = new HashSet<>(userStorage.findUserById(id).getFriends());
        Set<Integer> friendsSet = new HashSet<>(userStorage.findUserById(friendId).getFriends());
